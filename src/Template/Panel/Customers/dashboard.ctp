...
<div class="row">
    <div class="col-md-6">
        ...
        <span><?php echo h(__d('webshop', 'Address details')); ?>
            : <?php echo $this->Html->link(__d('webshop', 'View all'), array('controller' => 'AddressDetails', 'action' => 'index'), array('escape' => false)); ?></span>
    </div>
    <div class="col-md-6">
        <h3>
        ...
        <?php foreach (Nav::items('webshop-customer-dashboard') as $options): ?>
            <?php echo $this->Html->link($options['before'] . $options['title'] . $options['after'], $options['url'], array('escape' => false, 'class' => 'btn btn-default')); ?>
            <br/>
        <?php endforeach; ?>
    </div>
</div>
